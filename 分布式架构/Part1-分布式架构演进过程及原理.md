...
>
> 5.https://www.cnblogs.com/fengyc/p/12286726.html （AKF 扩展立方体）
...
### 微服务的拆分（AKF扩展立方体）
   ...
   * Z轴：数据分片
     * z 轴扩展是基于数据集本身的特性来进行扩展的方式，也即数据分片;
     * eg: 分表
...
### 微服务的容错模式
   ...
   *  舱壁隔离模式
      ...
      用航船的设计比喻舱壁隔离模式， 若一艘航船遇到了意外事故， 其中一个船舱进了水， 我们希望这个船舱和其它船舱是隔离的， 这样其它船舱就不会进水，不受影响。微服务架构中，主要体现在下面两个方面：

       * 微服务容器分组
       
       eg1: 支付平台， 将VIP商户和大部分流量路由到服务的核心池中， 少部分普通商户路由到灰度环境， 在项目重构时，先在灰度环境进行验证，成功了再部署到核心池； 
       eg2: 在社交平台， 将名人的自媒体流量路由到服务的核心池中， 普通用户的流量路由到另外的服务池中， 隔离普通用户和重要用户的负载； 
